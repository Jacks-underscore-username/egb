...
;(async () => {
  if (
    ...
    if (fs.existsSync(path.join(__dirname, 'config.json'))) {
      ...
      if (
        !['user', 'repo', 'tempFolder', 'ignoredPaths', 'passphrase'].every(
    ...
    ;((filePath, commandName) => {
      ...
      if (osType === 'win32') {
        ...
        if (
          fs.existsSync(batFilePath) &&
          fs.readFileSync(batFilePath, 'utf8') === batFileContent
        )
          return false
        ...
        execSync(
          'powershell -Command "$env:Path = [System.Environment]::GetEnvironmentVariable(\'Path\', [System.EnvironmentVariableTarget]::User)"'
        )
        return true
      }
      if (['linux', 'android', 'darwin'].includes(osType)) {
        ...
        if (bashrcContent.split('\n').includes(aliasCommand)) return false
        ...
        console.log(
          `Success! Added ${commandName} alias. Run 'source ~/.bashrc' or restart your terminal to apply the changes.`
        )
        return true
      }
    ...
    console.log(
      'Config file generated, finish setup by filling it out, then run egb save / egb load to use.'
    )
    process.exit(0)
  }

  const formatPath = (inputPath, slash = '/') =>
    inputPath.replace(/\/+|\\+/g, slash)
  const localPath = (() =>
    os.platform() === 'win32'
      ? (inputPath, slash = '\\') => formatPath(inputPath, slash)
      : (inputPath, slash = '/') => formatPath(inputPath, slash))()

  const { user, repo, tempFolder, ignoredPaths, passphrase } = JSON.parse(
    fs.readFileSync(path.join(__dirname, 'config.json'))
  )
  ...
  if (mode !== 'save' && mode !== 'load') {
    ...
    fs.rmSync(path.join(__dirname, tempFolder), {
      recursive: true,
      force: true
    })
    process.exit(1)
  ...
  const prettySize = bytes =>
    bytes >= 1024 ** 3
      ? `${(bytes / 1024 ** 3).toPrecision(4)}G`
      : bytes >= 1024 ** 2
        ? `${(bytes / 1024 ** 2).toPrecision(4)}m`
        : bytes >= 1024
          ? `${(bytes / 1024).toPrecision(4)}k`
          : `${bytes}b`
  ...
  const decryptFile = (inputPath, outputPath) =>
    new Promise(resolve => {
      ...
      const startDecryption = () => {
        if (
          !fs.existsSync(
            localPath(path.dirname(path.join(__dirname, outputPath)))
          )
        )
          fs.mkdirSync(
            localPath(path.dirname(path.join(__dirname, outputPath))),
        ...
        const output = fs.createWriteStream(
          localPath(path.join(__dirname, outputPath))
        )
      ...
      rl.on('line', line => {
        if (lineIndex === 0) {
          ...
          key = crypto.scryptSync(
            passphrase,
            Buffer.from(line, 'base64').toString('binary'),
            32
          )
        } else if (lineIndex === 1) {
          offset += Buffer.byteLength(Buffer.from(line)) + 1
  ...
  try {
    try {
      ...
      if (os.platform() === 'win32') execSync('ping -n 1 -l 1 github.com')
      else execSync('ping -c 1 -s 1 -w 5 github.com')
      clearTimeout(handle)
      if (toldOfPinging) console.log('Got response')
    } catch (e) {
      console.error(
        'Cannot connect to github, check your internet connection or https://githubstatus.com'
      )
      process.exit(1)
    }
    ...
    try {
      ...
      extraRepoFiles.forEach(filePath =>
        fs.copyFileSync(path.join(__dirname, filePath), filePath)
      )
      execSync('mkdir data', { stdio: 'inherit' })
      fs.writeFileSync(
        path.join(__dirname, tempFolder, 'data', 'INDEX.json'),
        JSON.stringify({
          blobs: {},
          files: [],
          currentFiles: [],
      ...
      await encryptFile(
        path.join(tempFolder, 'data', 'INDEX.json'),
        path.join(tempFolder, 'data', 'index')
      )
      execSync('git add *', { stdio: 'inherit' })
      execSync('git commit -m "First commit"', { stdio: 'inherit' })
      fs.rmSync(path.join(__dirname, tempFolder, 'data'), {
        recursive: true,
        force: true
      })
      extraRepoFiles.forEach(filePath =>
        fs.rmSync(path.join(__dirname, tempFolder, filePath))
      )
      execSync(`gh repo create ${user}/${repo} --public`, { stdio: 'inherit' })
      execSync(
        `git remote add origin https://github.com/${user}/${repo.toLowerCase()}.git`,
        { stdio: 'inherit' }
      )
    ...
    if (fs.existsSync(path.join(__dirname, `${repo.toLowerCase()}.git`))) {
      ...
      execSync(`git --git-dir=${repo.toLowerCase()}.git fetch origin main`, {
        stdio: 'inherit'
      })
      execSync(
        `git --git-dir=${repo.toLowerCase()}.git update-ref refs/heads/main refs/remotes/origin/main`,
        { stdio: 'inherit' }
      )
    } else {
      console.log(
        `No local repo found, cloning from https://github.com/${user}/${repo}`
      )
      execSync(
        `git clone --bare https://github.com/${user}/${repo.toLowerCase()}.git`,
        { stdio: 'inherit' }
      )
      execSync(
        `git --git-dir=${repo.toLowerCase()}.git config remote.origin.fetch "+refs/heads/*:refs/remotes/origin/*"`
      )
    }
  ...
  const index = await (async () => {
    execSync(
      `git --git-dir=${repo.toLowerCase()}.git show HEAD:data/index > "${path.join(__dirname, tempFolder, 'encryptedIndex')}"`,
      { stdio: 'inherit' }
    )
  ...
  const { differences, localFiles } = await (async () => {
    ...
    const scanFolder = async folderPath => {
      const extraIgnores = [
        '.egb_ignore',
        ...((fs.existsSync(path.join(__dirname, folderPath, '.egb_ignore')) &&
          (
            await promisify(fs.readFile)(
              path.join(__dirname, folderPath, '.egb_ignore'),
              'utf8'
            )
          )
            .split('\r\n')
      ...
      await Promise.all(
        (await promisify(fs.readdir)(path.join(__dirname, folderPath))).map(
          item =>
            (async () => {
              const itemPath = path.join(folderPath, item)
              const stat = await promisify(fs.stat)(
                path.join(__dirname, itemPath)
              )
              const formattedPath = formatPath(itemPath)
              if (stat.isDirectory()) {
                if (
                  !ignoredPaths.includes(item) &&
                  (itemPath !== `${repo.toLowerCase()}.git`) &
                    (itemPath !== tempFolder) &&
                  !extraIgnores.includes(item)
                )
                ...
                filePromises.push(
                  (async () => {
                    if (
                      !selfFiles.includes(itemPath) &&
                      !extraIgnores.includes(item)
                    ) {
                      let hash
                      if (repoPaths.includes(formattedPath)) {
                        const entry = index.currentFiles.find(
                          entry => entry.path === formattedPath
                        )
                        if (
                          Math.round(stat.mtimeMs) !== entry.mtime ||
    ...
    console.log(
      `Found ${differences.length} change${differences.length === 1 ? '' : 's'} and ${localFiles.length} local file${localFiles.length === 1 ? '' : 's'}`
    )
  ...
  const save = async () => {
          ...
          if (index.blobs[entry.hash] === undefined) {
            ...
            const blob = (
              await promisify(exec)(
                `git hash-object -w "${path.join(__dirname, tempFolder, name)}"`
              )
            ).stdout
              .toString()
              .trim()
    ...
    const savedFileMap = Object.fromEntries(
      index.files.map(entry => [entry.path, entry])
    )
    localFiles.forEach(entry => {
      if (savedFileMap[entry.path] === undefined)
        console.error(
          savedFileMap[entry.path],
          entry,
          savedFilePaths.includes(entry.path)
        )
    ...
    fs.writeFileSync(
      path.join(__dirname, tempFolder, 'blobs'),
      index.currentFiles
        .map(
          entry =>
            `100644 blob ${index.blobs[entry.hash]}\t${entry.randomName}\n`
        )
        .join(''),
      'utf8'
    )
    ...
    const commitMessage = (() => {
      ...
      const multipleTypes =
        differences.filter(
          (entry, index, arr) =>
            arr.slice(0, index).filter(subEntry => subEntry.type === entry.type)
              .length === 0
        ).length > 1
      if (multipleTypes) {
    ...
    const indexBlob = execSync(
      `git hash-object -w "${path.join(__dirname, tempFolder, 'index')}"`
    )
      .toString()
      .trim()
    ...
    fs.appendFileSync(
      path.join(__dirname, tempFolder, 'blobs'),
      `100644 blob ${indexBlob}\tindex`
    )

    const dataTreeHash = execSync(
      `git mktree < "${path.join(__dirname, tempFolder, 'blobs')}"`
    )
      .toString()
      .trim()
    fs.rmSync(path.join(__dirname, tempFolder, 'blobs'))

    const extraRepoBlobs = extraRepoFiles.map(filePath => ({
      hash: execSync(`git hash-object -w "${path.join(__dirname, filePath)}"`)
        .toString()
        .trim(),
      path: filePath
    }))
    ...
    fs.writeFileSync(
      path.join(__dirname, tempFolder, 'rootBlobs'),
      `040000 tree ${dataTreeHash}\tdata\n${extraRepoBlobs.map(entry => `100644 blob ${entry.hash}\t${entry.path}\n`).join('')}`,
      'utf8'
    )

    const rootTreeHash = execSync(
      `git mktree < "${path.join(__dirname, tempFolder, 'rootBlobs')}"`
    )
      .toString()
      .trim()
    fs.rmSync(path.join(__dirname, tempFolder, 'rootBlobs'))

    const latestCommitHash = fs.existsSync(
      path.join(__dirname, `${repo.toLowerCase()}.git`, 'refs/heads/main')
    )
      ? fs
          .readFileSync(
            path.join(
      ...
      : fs
          .readFileSync(
            path.join(__dirname, `${repo.toLowerCase()}.git`, 'packed-refs'),
            'utf8'
          )
          .match(/([0-9a-f]{40}) refs\/heads\/main/)[1]

    const commitHash = execSync(
      `git commit-tree ${rootTreeHash} -p ${latestCommitHash} -m ${crypto.randomBytes(8).toString('hex')} `
    )
      .toString()
      .trim()
    ...
    if (sendingSize)
      console.log(
        `Sending commit with ${prettySize(sendingSize)} of new content (${sendingFiles} file${sendingFiles === 1 ? '' : 's'})`
      )
    else console.log('Sending commit')

    execSync('git push origin refs/heads/main:refs/heads/main', {
      stdio: 'inherit'
    })
  ...
  const load = async () => {
    await Promise.all(
      differences.map(async difference => {
        const entry = index.currentFiles.find(
          entry => entry.path === difference.path
        )
        if (difference.type === 'remote') {
          await promisify(exec)(
            `git --git-dir=${repo.toLowerCase()}.git show HEAD:data/${entry.randomName} > "${path.join(__dirname, tempFolder, entry.randomName)}"`
          )
          await decryptFile(
            path.join(tempFolder, entry.randomName),
            difference.path
          )
          await promisify(fs.rm)(
            path.join(__dirname, tempFolder, entry.randomName)
          )
          await promisify(fs.utimes)(
            path.join(__dirname, difference.path),
            new Date(),
